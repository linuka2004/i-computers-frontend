...
export default function AdminUpdateProductPage() {
  ...
  async function updateProduct( ){
    ...
    let images = await Promise.all(imagePromises) // Wait for all uploads to complete
    .catch((err)=>{
    ...
    if(images.length == 0){
      images = location.state.images;  // If no new images were uploaded, retain the old images
    }
    ...
    <div className="w-full h-full flex justify-center items-start p-[50px] overflow-y-scroll">
      <div className="w-[800px] p-[40px] bg-accent/85 rounded-2xl shadow-2xl overflow-y-visible">
        ...
        <div className="w-full bg-white p-[20px] rounded-xl  flex flex-wrap justify-between shadow-2xl">
          ...
          <button onClick={updateProduct} className="w-[49%] h-[50px] bg-black text-white rounded-2xl hover:bg-accent/80 mt-[20px]">Update Product</button>
        </div>
      </div>
    </div>
  );
